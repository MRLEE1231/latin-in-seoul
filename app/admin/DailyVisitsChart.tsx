...
export default function DailyVisitsChart({ data }: { data: DailyVisitItem[] }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-gray-50/50 p-4">
      <h3 className="text-sm font-semibold text-gray-700 mb-3">일별 방문자 수</h3>
      <div ref={scrollRef} className="overflow-x-auto overflow-y-hidden pb-2 -mx-1 px-1" style={{ scrollbarGutter: 'stable' }}>
        <div className="flex items-end gap-1 min-h-40" style={{ minWidth: 'min-content', width: data.length ? data.length * BAR_WIDTH + (data.length - 1) * GAP : 'auto' }}>
          {data.length === 0 ? (
            <p className="text-sm text-gray-400 self-center py-8">데이터 없음</p>
          ) : (
            data.map((d) => {
              ...
              return (
                <div key={d.date} className="flex flex-col items-center gap-1 flex-shrink-0" style={{ width: BAR_WIDTH }}>
                  <div className="min-h-[18px] text-xs font-semibold text-slate-600 text-center">
                    {isSelected ? `${d.count}명` : '\u00A0'}
                  </div>
                  <button
                    type="button"
                    onClick={() => setSelectedDate(isSelected ? null : d.date)}
                    className="w-8 rounded-t bg-slate-500 hover:bg-slate-600 transition-colors flex-shrink-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-1"
                    style={{ height: `${barHeight}px` }}
                    title={`${d.date}: ${d.count}명`}
                  />
                  <span className="text-[10px] text-gray-500 text-center whitespace-nowrap" title={d.date}>
                    {formatShort(d.date)}
                  </span>
                </div>
              );
      ...
      <p className="text-xs text-gray-400 mt-2">최근 30일, 가로 스크롤로 일자 이동 · 당일 1회 쿠키 기준</p>
    </div>
  );
}
