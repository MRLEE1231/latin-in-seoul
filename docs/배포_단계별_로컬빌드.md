...
여태까지 반영한 내용(광고 관리, 이미지 압축, 메인 레이아웃 등)을 **로컬에서 빌드한 뒤 서버에 반영**하는 순서입니다.
...
로컬에 만든 `latin-in-seoul.tar`를 서버 홈(`~/`)으로 옮깁니다.

### 방법 A: scp (PowerShell)

```powershell
scp -i "C:\경로\개인키파일.key" latin-in-seoul.tar ubuntu@서버공인IP:~/
```

- `C:\경로\개인키파일.key` → 본인 SSH 개인키 경로
- `서버공인IP` → Oracle Cloud VM 공인 IP

### 방법 B: WinSCP 등 GUI 도구

- `latin-in-seoul.tar`를 서버의 `~/` (홈 디렉터리)로 업로드합니다.
