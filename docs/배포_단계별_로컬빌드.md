...
$env:DATABASE_URL = "postgresql://postgres:latinAdmin2026!@latin-postgres:5432/latin_in_seoul"
...
- **광고 이미지(기타) 업로드:** `public/ads` 볼륨을 쓰는 경우, 컨테이너가 쓸 수 있도록 **한 번만** 아래를 실행하세요 (앱 디렉터리에서).
  ```bash
  mkdir -p public/ads && sudo chown 1001:1001 public/ads
  ```
  그 다음 `docker run`을 다시 실행하면 됩니다.
...
### 기존 업로드 이미지 5MB 이하로 일괄 압축 (서버)

이미 올라간 수업 이미지 중 5MB 넘는 파일만 압축하려면, **새 이미지로 배포한 뒤** 서버에서 한 번 실행:

```bash
docker exec latin-app node /app/scripts/compress-uploaded-images.mjs /app/public/uploads
```

- `data/uploads` 볼륨이 `/app/public/uploads`로 붙어 있으므로, 위 명령이 서버의 `data/uploads` 안 파일을 압축합니다.
- 확장자가 `.png` → `.jpg`로 바뀐 경우 DB(`post_images.image_url`)도 자동으로 갱신됩니다.

---

### 광고 등록 시 `EACCES: permission denied, open '/app/public/ads/...'`

- **원인:** 서버에서 `public/ads` 볼륨 디렉터리가 root 소유라서 컨테이너 안의 `nextjs`(uid 1001)가 쓸 수 없음.
- **해결 (서버에서 한 번만):**
  ```bash
  cd ~/latin-in-seoul   # 실제 앱 경로로
  mkdir -p public/ads
  sudo chown 1001:1001 public/ads
  ```
  그 다음 앱 컨테이너만 다시 띄우면 됩니다.
  ```bash
